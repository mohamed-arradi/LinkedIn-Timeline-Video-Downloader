class LinkedinParser {
    ...
    cleanUpDuplicate(videoID, element) {
    ...
    addVideoLink() {
        ...
        document.getElementsByClassName(feedCardClass);
        var allVideos = Array.from(document.getElementsByClassName(feedCardClass));

        if (allVideos.length > 0) {
            Array.from(allVideos).forEach(element => {
                    ...
                    data.forEach(subElement => {
                        ...
                        if (listOptions !== undefined &&
                            ...
                            if (listOptionsArray[0].className.startsWith("option")) {
                                ...
                                var numberElements = dropDown.querySelectorAll("#" + videoID);

                                if (elementIsNotPresent === true &&
                                    Array.from(numberElements).length == 0) {
...
var linkedinParser = new LinkedinParser();
linkedinParser.addVideoLink()